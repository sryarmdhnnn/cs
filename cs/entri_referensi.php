...
<!--close-top-serch-->
<!--sidebar-menu-->
<div id="sidebar"><a href="entri_referensi.php" class="visible-phone"><i class="icon icon-tasks"></i> <span>Referensi</span></a>
  <ul>
    <?php
    if($r['id_level'] == 1){
  ?>
    ...
    <li> <a href="entri_order.php"><i class="icon icon-shopping-cart"></i> <span>Order</span></a> </li>
...
<!--breadcrumbs-->
  <div id="content-header">
    <div id="breadcrumb"> <a href="entri_referensi.php" title="Go to here" class="tip-bottom"><i class="icon icon-tasks"></i>Referensi</a></div>
  </div>
<!--End-breadcrumbs-->
          ...
          <ul class="thumbnails">
            <div class="btn-icon-pg">
              <ul>
                <!--Looping-->
                <?php
                  $query_data_minuman = "select * from tb_minuman order by id_minuman desc";
                ...
                ?>
                    <li class="span2"> 
                      ...
                      <form action="" method="post">
                        <button type="submit" value="<?php echo $r_dt_minuman['id_minuman'];?>" name="edit_menu" class="btn btn-success btn-mini"><i class='icon-pencil'></i>&nbsp;&nbsp;Edit &nbsp;&nbsp;</button>
                        <button type="submit" value="<?php echo $r_dt_minuman['id_minuman'];?>" name="hapus_menu" class="btn btn-mini btn-danger"><i class='icon icon-trash'></i>&nbsp; Hapus</button>
                      </form>
                    </li>
                  <?php
                    }
                    if(isset($_REQUEST['hapus_menu'])){
                      ...
                      $id_minuman = $_REQUEST['hapus_menu'];

                      $query_lihat = "select * from tb_minuman where id_minuman = $id_minuman";
                      $sql_lihat = mysqli_query($conn, $query_lihat);
                      $result_lihat = mysqli_fetch_array($sql_lihat);
                      if(file_exists('gambar/'.$result_lihat['gambar_minuman'])){
                        //echo $result_lihat['gambar_minuman'];
                        unlink('gambar/'.$result_lihat['gambar_minuman']);
                      }
                      $query_hapus_minuman = "delete from tb_minuman where id_minuman = $id_minuman";
                      $sql_hapus_minuman= mysqli_query($conn, $query_hapus_minuman);
                      if($sql_hapus_minuman){
                    ...
                    if(isset($_REQUEST['edit_menu'])){
                      //echo $_REQUEST['hapus_menu'];
                      $id_minuman = $_REQUEST['edit_menu'];
                      $_SESSION['edit_menu'] = $id_minuman;
