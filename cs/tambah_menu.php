...
if(isset ($_SESSION['username'])){
  ...
  $harga = "";
  $stok = "";
  $gambar_minuman = "no_image.png";

  if(isset($_SESSION['edit_menu'])){
    $id = $_SESSION['edit_menu'];
    $query_data_edit = "select * from tb_minuman where id_minuman = $id";
    $sql_data_edit = mysqli_query($conn, $query_data_edit);
    $result_data_edit = mysqli_fetch_array($sql_data_edit);

    $id_minuman = $result_data_edit['id_minuman'];
    $nama_minuman = $result_data_edit['nama_minuman'];
    $harga = $result_data_edit['harga'];
    $stok = $result_data_edit['stok'];
    $gambar_minuman = $result_data_edit['gambar_minuman'];
  }
...
<div id="sidebar">
  ...
  <ul>
    ...
    <li> <a href="widgets.html"><i class="icon icon-shopping-cart"></i> <span>Order</span></a> </li>
    <li> <a href="widgets.html"><i class="icon icon-inbox"></i> <span>Transaksi</span></a> </li>
      ...
      <div class="widget-box span6">
        ...
        <div class="widget-content" >
          <form action="" method="post" class="form-horizontal" accept-charset="UTF-8" enctype="multipart/form-data">
            <div class="control-group">
              ...
              <div class="controls">
                <?php 
                  if(isset($_SESSION['edit_menu'])){
                ?>
                  <input name="" type="text" value="<?php echo $nama_minuman; ?>" class="span11" placeholder="Nama minuman" disabled=""/>
                  <input name="nama_minuman" type="hidden" value="<?php echo $nama_minuman; ?>" class="span11" placeholder="Nama minuman"/>
                <?php
                  } else {
                ?>
                  <input name="nama_minuman" type="text" value="" class="span11" placeholder="Nama minuman"/>
                <?php
                  }
                ?>
              </div>
            ...
            <div class="control-group">
              <label class="control-label">Gambar minuman :</label>
              <div class="control-group">
                <div class="controls">
            ...
            <div class="control-group">
              ...
              <div class="control-group">
                <div class="controls">
                  <img src="gambar/<?php echo $gambar_minuman;?>" id="previewne" class="rounded border p-1" style="width:110px; height:70px;">
                </div>
              </div>
            </div>
          ...
          <?php
            if(isset($_POST['tambah_menu'])){
              ...
              $nama_baru = $_POST['nama_minuman'].".".$name;
              ...
              if($stok > 0){
                $status_minuman = 'tersedia';
              } else {
                $status_minuman = 'habis';
              }
              //echo "<br>";
              //echo $nama_user . " || " . $username . " || " . $password . " || " . $id_level . " || " . $status;
              //echo "<br></br>";
              $query_tambah_minuman = "insert into tb_minuman values ('','$nama_minuman','$harga','$stok','$status_minuman','$gambar')";
              $sql_tambah_minuman= mysqli_query($conn, $query_tambah_minuman);
              if($sql_tambah_minuman){
            ...
            if(isset($_POST['ubah_menu'])){
              ...
              $query_ubah_minuman = "update tb_minuman set nama_minuman = '$nama_minuman', harga = '$harga', stok = '$stok', status_minuman = '$status_minuman' where id_minuman = '$id_minuman'";;
              $sql_ubah_minuman = mysqli_query($conn, $query_ubah_minuman);
              ...
              if($gbr != "" || $gbr != null){
                ...
                $name = pathinfo($_FILES["gambar"]["name"], PATHINFO_EXTENSION);
                $nama_baru = $_POST['nama_minuman'].".".$name;
                unlink('gambar/'.$gambar_minuman);
                move_uploaded_file($tmp_name, $direktori."/".$nama_baru);
                $gambar = $nama_baru;

                $query_ubah_gambar = "update tb_minuman set gambar_minuman = '$gambar' where id_minuman = '$id_minuman'";;
