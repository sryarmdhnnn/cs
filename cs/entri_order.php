...
<!--Header-part-->
<div id="header">
  <h1><a href="entri_order.php">Order</a></h1>
</div>
<!--close-Header-part--> 
...
<!--close-top-serch-->
<!--sidebar-menu-->
<div id="sidebar"><a href="entri_order.php" class="visible-phone"><i class="icon shopping-cart"></i> <span>Order</span></a>
  <ul>
    <?php
    if($r['id_level'] == 1){
  ?>
    <li> <a href="beranda.php"><i class="icon icon-home"></i> <span>Home</span></a> </li>
    <li> <a href="entri_referensi.php"><i class="icon icon-tasks"></i> <span> Referensi</span></a> </li>
    <li class="active"> <a href="entri_order.php"><i class="icon icon-shopping-cart"></i> <span>Order</span></a> </li>
...
<!--breadcrumbs-->
  <div id="content-header">
    <div id="breadcrumb"> <a href="entri_order.php" title="Go to here" class="tip-bottom"><i class="icon icon-tasks"></i>  Order</a></div>
  </div>
<!--End-breadcrumbs-->
          ...
          <div class="widget-content nopadding">
            <table class="table table-bordered table-invoice-full">
              ...
              <tbody>
                <?php
                  $no_order_fiks = 1;
                  $query_order_fiks = "select * from tb_pesan natural join tb_minuman where id_user = $id and status_pesan != 'sudah'";
                ...
                <tr>
                  ...
                  <td><?php echo $r_order_fiks['nama_minuman'];?></td>
    ...
    <div class="row-fluid">
              ...
              <div class="btn-icon-pg">
                <ul>
                  ...
                  <?php
                    ...
                    $query_data_minuman = "select * from tb_minuman where stok > 0 order by id_minuman desc";
                    $sql_data_minuman = mysqli_query($conn, $query_data_minuman);
                    $no_minuman = 1;

                    while($r_dt_minuman = mysqli_fetch_array($sql_data_minuman)){
                  ?>
                      <li class="span2"> 
                        <a> <img src="gambar/<?php echo $r_dt_minuman['gambar_minuman']?>" alt="" > </a>
                        <div class="actions">
                          <a class="lightbox_trigger" href="gambar/<?php echo $r_dt_minuman['gambar_minuman'];?>"><i class="icon-search"></i>&nbsp;Lihat</a> 
                        </div>
                        <table class="table table-bordered">
                          <tbody>
                            <tr>
                              <td><?php echo $r_dt_minuman['nama_minuman'];?></td>
                            </tr>
                            <tr>
                              <td>Harga / Porsi</td>
                              <td>Rp. <?php echo $r_dt_minuman['harga'];?>,-</td>
                            </tr>
                            <tr>
                              <td>Stok</td>
                              <td><?php echo $r_dt_minuman['stok'];?> Porsi</td>
                            </tr>
                          </tbody>
                        </table>
                        <form action="" method="post">
                          <?php
                            if(in_array($r_dt_minuman['id_minuman'], $pesan)){
                          ?>
                              <button type="submit" value="<?php echo $r_dt_minuman['id_minuman'];?>" name="tambah_pesan" class="btn btn-danger btn-mini" disabled>
                                <i class='icon-shopping-cart'></i>&nbsp;&nbsp;Telah dipesan &nbsp;&nbsp;
                              </button>
                          <?php
                            } else {
                          ?>
                              <button type="submit" value="<?php echo $r_dt_minuman['id_minuman'];?>" name="tambah_pesan" class="btn btn-success btn-mini">
                    ...
                    <?php
                      ...
                      if(isset($_REQUEST['tambah_pesan'])){
                        ...
                        $id_minuman = $_REQUEST['tambah_pesan'];

                        $query_tambah_pesan = "insert into tb_pesan values('', '$id', '', '$id_minuman', '', '')";
        ...
        <div class="widget-box">
          ...
          <div class="widget-content nopadding">
            <table class="table table-bordered table-striped">
              ...
              <tbody>
              <?php
                $query_draft_pesan = "select * from tb_pesan natural join tb_minuman where id_user = $id and status_pesan != 'sudah'";
                $sql_draft_pesan = mysqli_query($conn, $query_draft_pesan);

                while($r_draft_pesan = mysqli_fetch_array($sql_draft_pesan)){
              ?>
                <tr class="odd gradeX">
                  <td><span id="<?php echo "nama".$r_draft_pesan['id_pesan']; ?>"><?php echo $r_draft_pesan['nama_minuman'];?></span></td>
                  <input id="<?php echo "harga".$r_draft_pesan['id_pesan']; ?>" class="span8" type="hidden" value="<?php echo $r_draft_pesan['harga'];?>"/>
                  <td>
                    <center>
                      <input id="<?php echo "jumlah".$r_draft_pesan['id_pesan']; ?>" class="span8" name="jumlah<?php echo $r_draft_pesan['id_minuman']; ?>" type="number" value="" placeholder="" onchange="return operasi()"/>
                    </center>
                  </td>
                  <td>
                    <form action="" method="post">
      ...
      <?php
          ...
          if(isset($_POST['proses_pesan'])){
            ...
            $query_ubah_jumlah = "select * from tb_pesan left join tb_minuman on tb_pesan.id_minuman = tb_minuman.id_minuman where id_user = $id and status_pesan != 'sudah'";
            $sql_ubah_jumlah = mysqli_query($conn, $query_ubah_jumlah);
            while($r_ubah_jumlah = mysqli_fetch_array($sql_ubah_jumlah)){
              $tahu = $r_ubah_jumlah['id_minuman'];
              $tempe = $_POST['jumlah'.$tahu];
              $id_pesan = $r_ubah_jumlah['id_pesan'];
              $query_stok = "select * from tb_minuman where id_minuman = $tahu";
              $sql_stok = mysqli_query($conn, $query_stok);
              $result_stok = mysqli_fetch_array($sql_stok);
              $sisa_stok = $result_stok['stok'] - $tempe;
              //echo $tempe;
              $query_proses_ubah = "update tb_pesan set jumlah = $tempe, id_order = $id_ordernya where id_minuman = $tahu and id_user = $id and status_pesan != 'sudah'";
              $query_kurangi_stok = "update tb_minuman set stok = $sisa_stok where id_minuman = $tahu";
...
<div class="row-fluid">
  <div id="footer" class="span12"> <?php echo date('Y'); ?> &copy; CAFE <a href="#">by Suryarmdhnnn</a> </div>
</div>
